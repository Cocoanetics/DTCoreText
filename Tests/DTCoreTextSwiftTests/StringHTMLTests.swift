...
struct StringHTMLTests {
	...
	func emojiEncodingAndDecoding() {
		...
		let encoded = string.stringByAddingHTMLEntities()
		#expect(encoded == "&#128516;")

		let decoded = (encoded as NSString).stringByReplacingHTMLEntities()
		#expect(decoded == string as String)
	}

	@Test("Hex entity decoding")
	func hexDecoding() {
		let decoded = ("&#x1F604;" as NSString).stringByReplacingHTMLEntities()
		#expect(decoded == "😄")
	}
	...
	func appleConvertedSpace() {
		...
		let s1 = ("1 1" as NSString).stringByAddingAppleConvertedSpace()
		#expect(s1 == "1 1")

		// Two spaces
		let s2 = ("2  2" as NSString).stringByAddingAppleConvertedSpace()
		let expected2 = "2 <span class=\"Apple-converted-space\">\(nbsp)</span>2"
		#expect(s2 == expected2)

		// Three spaces
		let s3 = ("3   3" as NSString).stringByAddingAppleConvertedSpace()
		let expected3 = "3 <span class=\"Apple-converted-space\">\(nbsp) </span>3"
		#expect(s3 == expected3)

		// Four spaces
		let s4 = ("4    4" as NSString).stringByAddingAppleConvertedSpace()
