...
@interface DTTextAttachmentImage () // private stuff
...
@implementation DTTextAttachmentImage
{
	DTImage *_image;
}
...
- (void)_decodeImageSrc:(NSString *)src relativeToBaseURL:(NSURL *)baseURL
{
	...
	if (!_displaySize.width || !_displaySize.height)
	{
		...
		if (!_image)
		{
			image = [[DTTextAttachmentImage sharedImageCache] objectForKey:[contentURL absoluteString]];
		}
		...
		if (!image)
		{
			...
			if (image)
			{
				[[DTTextAttachmentImage sharedImageCache] setObject:image forKey:[contentURL absoluteString]];
			}
		}
...
- (DTImage *)image
{
	if (!_image)
	{
		if (_contentURL)
		{
			DTImage *image = [[DTTextAttachmentImage sharedImageCache] objectForKey:[_contentURL absoluteString]];
			...
			if (!image && [_contentURL isFileURL])
			{
				...
				if (image)
				{
					[[DTTextAttachmentImage sharedImageCache] setObject:image forKey:[_contentURL absoluteString]];
				}
			}
