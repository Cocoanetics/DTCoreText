...
extension NSAttributedString {
  ...
  public class func synthesizedSmallCapsAttributedString(
    withText text: String, attributes: NSDictionary
  ) -> NSAttributedString? {
    ...
    let smallAttributes = NSMutableDictionary(dictionary: attributes)
      ...
      let uiFont =
        UIFont(
          name: CTFontCopyPostScriptName(smallerFont) as String, size: CTFontGetSize(smallerFont))
        ?? UIFont.systemFont(ofSize: CTFontGetSize(smallerFont))
    ...
    while !scanner.isAtEnd {
      if let part = scanner.scanCharacters(from: lowerCaseChars) {
        ...
        let partString = NSAttributedString(
          string: upper, attributes: smallAttributes as? [NSAttributedString.Key: Any])
        tmpString.append(partString)
      }

      if let part = scanner.scanUpToCharacters(from: lowerCaseChars) {
        let partString = NSAttributedString(
          string: part, attributes: attributes as? [NSAttributedString.Key: Any])
        tmpString.append(partString)
      }
    }
