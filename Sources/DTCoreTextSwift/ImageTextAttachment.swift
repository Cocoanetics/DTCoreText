        ...
        if let decodedData = decodedData {
          ...
          if let stylesStr = (element.attributes as? [String: Any])?["style"] as? String {
              ...
              widthStr.hasSuffix("px"), heightStr.hasSuffix("px")
            {
              ...
              sizeAccordingToStyle.width = (widthStr as NSString).pixelSizeOfCSSMeasure(
                relativeToCurrentTextSize: 0, textScale: 1)
              sizeAccordingToStyle.height = (heightStr as NSString).pixelSizeOfCSSMeasure(
                relativeToCurrentTextSize: 0, textScale: 1)

              if let currentImage = decodedImage,
                sizeAccordingToStyle.width != 0,
                sizeAccordingToStyle.width < currentImage.size.width,
                sizeAccordingToStyle.height != 0,
                sizeAccordingToStyle.height < currentImage.size.height
              {
                ...
                if scale >= 2.0 && scale <= 5.0 {
                  #if canImport(UIKit)
                    decodedImage = DTImage(
                      cgImage: currentImage.cgImage!, scale: scale,
