...
open class CoreTextLayoutLine: NSObject {
    ...
    @objc open func justifiedLine(withFactor justificationFactor: CGFloat, justificationWidth: CGFloat) -> CoreTextLayoutLine? {
        guard let justifiedLine = CTLineCreateJustifiedLine(_line, justificationFactor, Double(justificationWidth)) else { return nil }
    ...
    @objc open func frameOfGlyphs(with range: NSRange) -> CGRect {
        ...
        var tmpRect = CGRect(x: CGFloat.greatestFiniteMagnitude, y: CGFloat.greatestFiniteMagnitude, width: 0, height: 0)
        ...
        for oneRun in runs {
            ...
            if glyphFrame.size.height > tmpRect.size.height { tmpRect.size.height = glyphFrame.size.height }
    ...
    @objc open func isHorizontalRule() -> Bool {
        ...
        return (singleRun.attributes as? [NSAttributedString.Key: Any])?[NSAttributedString.Key(rawValue: DTHorizontalRuleStyleAttribute)] != nil
    }
    ...
    private func _scanGlyphRunsForValues() {
        ...
        for oneRun in runs {
            if let usedFont = (oneRun.attributes as? [NSAttributedString.Key: Any])?[NSAttributedString.Key(rawValue: kCTFontAttributeName as String)] as! CTFont? {
    ...
    @objc open var frame: CGRect {
        ...
        var frame = CGRect(x: baselineOrigin.x, y: baselineOrigin.y - _ascent, width: _width, height: _ascent + _descent)
    ...
    @objc open var textBlocks: NSArray? {
        guard let lastRun = (glyphRuns as? [CoreTextGlyphRun])?.last else { return nil }
        return (lastRun.attributes as? [NSAttributedString.Key: Any])?[NSAttributedString.Key(rawValue: DTTextBlocksAttribute)] as? NSArray
    }
