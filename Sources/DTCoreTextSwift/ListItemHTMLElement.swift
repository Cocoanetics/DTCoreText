...
open class ListItemHTMLElement: HTMLElement {
  ...
  private func _listPrefix() -> NSAttributedString? {
    let paragraphStyle = CoreTextParagraphStyle.paragraphStyle(
      withNSParagraphStyle: (attributesForAttributedStringRepresentation()
        as! [NSAttributedString.Key: Any])[.paragraphStyle] as? NSParagraphStyle)
    ...
    let attributes =
      tmpCopy.attributesForAttributedStringRepresentation() as! [NSAttributedString.Key: Any]
    ...
    let tmpStr = NSMutableAttributedString(
      string: prefix, attributes: newAttributes as? [NSAttributedString.Key: Any])
    ...
    if let image = image {
      ...
      if effectiveList.position == .inside {
        tmpStr.setAttributes(
          mutableNewAttributes as? [NSAttributedString.Key: Any],
    ...
    let tmpAttributedString = NSAttributedString(
      string: trimmedPrefix, attributes: newAttributes as? [NSAttributedString.Key: Any])
