...
open class CoreTextFontCollection: NSObject {
  ...
  @objc public func matchingFontDescriptor(for descriptor: CoreTextFontDescriptor)
    -> CoreTextFontDescriptor?
  {
    let cacheKey = NSString(
      format: "fontFamily BEGINSWITH[cd] %@ and boldTrait == %d and italicTrait == %d",
      descriptor.fontFamily ?? "", descriptor.boldTrait ? 1 : 0, descriptor.italicTrait ? 1 : 0)
    ...
    let predicate = NSPredicate(
      format: "fontFamily BEGINSWITH[cd] %@ and boldTrait == %d and italicTrait == %d",
      descriptor.fontFamily ?? "", NSNumber(value: descriptor.boldTrait),
      NSNumber(value: descriptor.italicTrait))
  ...
  @objc public func fontDescriptors() -> NSArray {
    ...
    guard
      let matchingFonts = CTFontCollectionCreateMatchingFontDescriptors(fonts)
        as? [CTFontDescriptor]
    else {
      return NSArray()
    }
