...
open class CoreTextGlyphRun: NSObject {
  ...
  @objc open func drawDecoration(in context: CGContext) {
    ...
    let backgroundColor = (self.attributes as? [NSAttributedString.Key: Any])?
      .dtct_backgroundColor()
    ...
    if drawStrikeOut || drawUnderline || backgroundColor != nil {
      ...
      let superscriptStyle =
        (_attributes?[NSAttributedString.Key(rawValue: kCTSuperscriptAttributeName as String)]
      ...
      if let backgroundColor = backgroundColor {
        let backgroundColorRect = CGRect(
          x: adjustedBounds.origin.x, y: lineFrame.origin.y, width: adjustedBounds.size.width,
      ...
      if drawStrikeOut || drawUnderline {
        ...
        if drawStrikeOut {
          ...
          if let ctFont = ctFont {
            ...
            y = DTRoundWithContentScale(
              adjustedBounds.origin.y + _ascent - strokePosition, contentScale)
          } else {
            y = DTRoundWithContentScale(
              adjustedBounds.origin.y + self.frame.size.height / 2.0 + 1, contentScale)
          }
        ...
        if drawUnderline && !DTCoreTextDrawsUnderlinesWithGlyphs() {
          ...
          var y = DTRoundWithContentScale(
            line.baselineOrigin.y + underlinePosition - fontUnderlineThickness / 2.0, contentScale)
  ...
  @objc open func newPathWithGlyphs() -> CGPath? {
    guard
      let font = (self.attributes as? [NSAttributedString.Key: Any])?[
    ...
    guard let glyphs = CTRunGetGlyphsPtr(_run),
      let positions = CTRunGetPositionsPtr(_run)
    else { return nil }
  ...
  private func calculateMetrics() {
    ...
    if !_didCalculateMetrics {
      _width = CGFloat(
        CTRunGetTypographicBounds(_run, CFRangeMake(0, 0), &_ascent, &_descent, &_leading))
  ...
  @objc open func frameOfGlyph(at index: Int) -> CGRect {
    ...
    var rect = CGRect(
      x: line.baselineOrigin.x + glyphPosition.x, y: line.baselineOrigin.y - _ascent,
      width: _offset + _width - glyphPosition.x, height: _ascent + _descent)
  ...
  @objc open func stringRange() -> NSRange {
    if _stringRange.length == 0 {
      ...
      _stringRange = NSRange(
        location: range.location + (_line?.stringLocationOffset ?? 0), length: range.length)
    }
    return _stringRange
  }
  ...
  @objc open var isHyperlink: Bool {
    if !_hyperlink && !_didCheckForHyperlinkInAttributes {
      _hyperlink =
        (self.attributes as? [NSAttributedString.Key: Any])?[
          NSAttributedString.Key(rawValue: DTLinkAttribute)] != nil
  ...
  @objc open var frame: CGRect {
    ...
    return CGRect(
      x: line.baselineOrigin.x + _offset, y: line.baselineOrigin.y - _ascent, width: _width,
