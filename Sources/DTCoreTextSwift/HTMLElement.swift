...
open class HTMLElement: HTMLParserNode {
    ...
    @objc public class func element(withName name: String, attributes: NSDictionary?, options: NSDictionary?) -> HTMLElement {
        ...
        if cls == nil {
            ...
            if let _ = TextAttachment.registeredClass(forTagName: name) {
    ...
    @objc open func attributesForAttributedStringRepresentation() -> NSDictionary {
        ...
        if let textAttachment = _textAttachment {
            ...
            tmpDict[NSAttributedString.Key(rawValue: kCTRunDelegateAttributeName as String)] = embeddedObjectRunDelegate
            #endif
            ...
            tmpDict[NSAttributedString.Key(rawValue: DTAttachmentParagraphSpacingAttribute)] = NSNumber(value: Double(self.paragraphStyle.paragraphSpacing))
        }
        ...
        if _strikeOut {
            tmpDict[NSAttributedString.Key.strikethroughStyle] = NSNumber(value: NSUnderlineStyle.single.rawValue)
        }

        // set underline style
        if _underlineStyle.rawValue != 0 {
            tmpDict[NSAttributedString.Key.underlineStyle] = NSNumber(value: NSUnderlineStyle.single.rawValue)
        }
        ...
        if _superscriptStyle != 0 {
            tmpDict[NSAttributedString.Key(rawValue: kCTSuperscriptAttributeName as String)] = NSNumber(value: _superscriptStyle)
        }
        ...
        if _headerLevel != 0 {
            tmpDict[NSAttributedString.Key(rawValue: DTHeaderLevelAttribute)] = NSNumber(value: _headerLevel)
        }
        ...
        if let backgroundStrokeColor = _backgroundStrokeColor {
            tmpDict[NSAttributedString.Key(rawValue: DTBackgroundStrokeColorAttribute)] = backgroundStrokeColor.cgColor
        }

        if _backgroundStrokeWidth != 0 {
            tmpDict[NSAttributedString.Key(rawValue: DTBackgroundStrokeWidthAttribute)] = NSNumber(value: Double(_backgroundStrokeWidth))
        }

        if _backgroundCornerRadius != 0 {
            tmpDict[NSAttributedString.Key(rawValue: DTBackgroundCornerRadiusAttribute)] = NSNumber(value: Double(_backgroundCornerRadius))
        }
    ...
    private func _isNotChildOfList() -> Bool {
        ...
        if parent.displayStyle == .block && (parent.name == "ol" || parent.name == "ul") { return false }
        return true
    }
    ...
    @objc open func attributedString() -> NSAttributedString? {
        ...
        if _textAttachment != nil {
            tmpString = NSMutableAttributedString(string: UNICODE_OBJECT_PLACEHOLDER, attributes: attributes)
        } else {
    ...
    private func _finishAttributedString(_ tmpString: NSMutableAttributedString, attributes: [NSAttributedString.Key: Any]) -> NSMutableAttributedString {
    ...
    private func _parseEdgeInsets(from styles: NSDictionary, forAttributesWithPrefix prefix: String, writingDirection: CTWritingDirection, intoEdgeInsets insets: inout UIEdgeInsets) -> Bool {
        ...
        for oneKey in stylesDict.keys {
            ...
            if oneKey.contains("-") {
                ...
                if oneKey.hasSuffix(leftKey) {
                    edgeInsets.left = (attributeValue as NSString).pixelSizeOfCSSMeasure(relativeToCurrentTextSize: fontSize, textScale: _textScale)
                    didModify = true
                } else if oneKey.hasSuffix(bottomKey) {
                    edgeInsets.bottom = (attributeValue as NSString).pixelSizeOfCSSMeasure(relativeToCurrentTextSize: fontSize, textScale: _textScale)
                    didModify = true
                } else if oneKey.hasSuffix(rightKey) {
                    edgeInsets.right = (attributeValue as NSString).pixelSizeOfCSSMeasure(relativeToCurrentTextSize: fontSize, textScale: _textScale)
                    didModify = true
                } else if oneKey.hasSuffix(topKey) {
                    edgeInsets.top = (attributeValue as NSString).pixelSizeOfCSSMeasure(relativeToCurrentTextSize: fontSize, textScale: _textScale)
                    didModify = true
                }
            } else {
                edgeInsets = (attributeValue as NSString).dtEdgeInsets(relativeToCurrentTextSize: _fontDescriptor?.pointSize ?? 12, textScale: _textScale)
                didModify = true
            }
        }
    ...
    @objc open func applyStyleDictionary(_ styles: NSDictionary) {
        ...
        if let fontSizeObj = styles["font-size"] {
            if let s = fontSizeObj as? String { fontSize = s }
            else if let arr = fontSizeObj as? [Any], let last = arr.last as? String { fontSize = last }
        }
        if let fontSize = fontSize {
            ...
            default:
                if (fontSize as NSString).isCSSLengthValue() {
                    fd?.pointSize = (fontSize as NSString).pixelSizeOfCSSMeasure(relativeToCurrentTextSize: _currentTextSize, textScale: _textScale)
                }
            }
        }
        ...
        if let fontFamilyStyle = styles["font-family"] {
            ...
            if let s = fontFamilyStyle as? String { fontFamilies = [s] }
            else if let arr = fontFamilyStyle as? [String] { fontFamilies = arr }
            ...
            if let families = fontFamilies {
                ...
                for fontFamily in families {
                    ...
                    if let font = _fontDescriptor?.newMatchingFont() {
                        ...
                        let lc = fontFamily.lowercased().replacingOccurrences(of: "\\s", with: "", options: .regularExpression)
                        let parts = lc.components(separatedBy: ",")

                        if parts.contains("sans-serif") || parts.contains("geneva") {
                            _fontDescriptor?.fontFamily = "Helvetica"; foundFontFamily = true
                        } else if parts.contains("serif") || parts.contains("times") {
                            _fontDescriptor?.fontFamily = "Times New Roman"; foundFontFamily = true
                        } else if parts.contains("monospace") {
                            _fontDescriptor?.monospaceTrait = true
                            _fontDescriptor?.fontFamily = "Courier"; foundFontFamily = true
                        } else if parts.contains("cursive") {
                            _fontDescriptor?.stylisticClass = CTFontStylisticClass.scriptsClass
                            _fontDescriptor?.fontFamily = nil; foundFontFamily = true
                        } else if parts.contains("fantasy") {
                            _fontDescriptor?.fontFamily = "Papyrus"; foundFontFamily = true
                        } else if lc == "inherit" {
                            _fontDescriptor?.fontFamily = self.parentElement()?.fontDescriptor.fontFamily; foundFontFamily = true
                        }
                    }
        ...
        if let decoration = (styles["text-decoration"] as? String)?.lowercased() {
            ...
            case "none": self.underlineStyle = CTUnderlineStyle(rawValue: 0); self.strikeOut = false
            default: break
            }
        }
        ...
        if let verticalAlignment = (styles["vertical-align"] as? String)?.lowercased() {
            ...
            case "baseline": self.superscriptStyle = 0; _textAttachmentAlignment = .baseline
        ...
        if let ls = (styles["letter-spacing"] as? String)?.lowercased() {
            if ls == "normal" { _letterSpacing = 0 }
            else if ls != "inherit" {
                _letterSpacing = (ls as NSString).pixelSizeOfCSSMeasure(relativeToCurrentTextSize: _fontDescriptor?.pointSize ?? 12, textScale: _textScale)
            }
        }
        ...
        if let shadow = styles["text-shadow"] {
            self.shadows = (shadow as? NSString)?.arrayOfCSSShadows(withCurrentTextSize: _fontDescriptor?.pointSize ?? 12, currentColor: _textColor) as [Any]?
        }
        ...
        if let lineHeight = (styles["line-height"] as? String)?.lowercased() {
            if lineHeight == "normal" {
                ...
                if (lineHeight as NSString).dt_isNumeric() {
                    ...
                    let lhValue = (lineHeight as NSString).pixelSizeOfCSSMeasure(relativeToCurrentTextSize: _fontDescriptor?.pointSize ?? 12, textScale: _textScale)
        ...
        if let widthString = styles["width"] as? String, widthString != "auto" {
            _size.width = (widthString as NSString).pixelSizeOfCSSMeasure(relativeToCurrentTextSize: _fontDescriptor?.pointSize ?? 12, textScale: _textScale)
        }
        if let heightString = styles["height"] as? String, heightString != "auto" {
            _size.height = (heightString as NSString).pixelSizeOfCSSMeasure(relativeToCurrentTextSize: _fontDescriptor?.pointSize ?? 12, textScale: _textScale)
        }
        ...
        if let textIndentStr = styles["text-indent"] as? String, (textIndentStr as NSString).isCSSLengthValue() {
            _pTextIndent = (textIndentStr as NSString).pixelSizeOfCSSMeasure(relativeToCurrentTextSize: _currentTextSize, textScale: _textScale)
        }

        let needsTextBlock = (_backgroundColor != nil || _backgroundStrokeColor != nil || _backgroundCornerRadius > 0 || _backgroundStrokeWidth > 0)

        var hasMargins = false
        let allKeys = (styles.allKeys as? [String])?.joined(separator: ";") ?? ""

        if allKeys.contains("-webkit-margin") {
            hasMargins = _parseEdgeInsets(from: styles, forAttributesWithPrefix: "-webkit-margin", writingDirection: self.paragraphStyle.baseWritingDirection, intoEdgeInsets: &_margins) || hasMargins
        }
        if allKeys.contains("margin") {
            hasMargins = _parseEdgeInsets(from: styles, forAttributesWithPrefix: "margin", writingDirection: self.paragraphStyle.baseWritingDirection, intoEdgeInsets: &_margins) || hasMargins
        }

        var hasPadding = false
        if allKeys.contains("-webkit-padding") {
            hasPadding = _parseEdgeInsets(from: styles, forAttributesWithPrefix: "-webkit-padding", writingDirection: self.paragraphStyle.baseWritingDirection, intoEdgeInsets: &_padding) || hasPadding
        }
        if allKeys.contains("padding") {
            hasPadding = _parseEdgeInsets(from: styles, forAttributesWithPrefix: "padding", writingDirection: self.paragraphStyle.baseWritingDirection, intoEdgeInsets: &_padding) || hasPadding
        }
    ...
    @objc open class func attributesToIgnoreForCustomAttributesAttribute() -> NSSet {
        return NSSet(array: ["style", "dir", "align", "src", "href", "color", "face", "size", "name", "height", "width"])
    }
