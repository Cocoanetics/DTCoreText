...
open class AnchorHTMLElement: HTMLElement {
    ...
    @objc open override func attributedString() -> NSAttributedString? {
        ...
        guard let mutableAttributedString = super.attributedString()?.mutableCopy() as? NSMutableAttributedString else {
            return nil
        }
        ...
        if let highlightedTextColor = highlightedTextColor {
            ...
            mutableAttributedString.addAttribute(NSAttributedString.Key(rawValue: DTLinkHighlightColorAttribute), value: highlightedTextColor, range: range)

            // we need to set the text color via the graphics context
            mutableAttributedString.addAttribute(NSAttributedString.Key(rawValue: kCTForegroundColorFromContextAttributeName as String), value: NSNumber(value: true), range: range)
        }
