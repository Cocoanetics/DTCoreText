...
private func _ensureInitialized() {
    ...
    let path = Bundle(for: CoreTextFontDescriptor.self).path(forResource: "DTCoreTextFontOverrides", ofType: "plist")
    ...
    if let path = path, let fileArray = NSArray(contentsOfFile: path) as? [[String: Any]] {
        for oneOverride in fileArray {
            guard let fontFamily = oneOverride["FontFamily"] as? String,
                  let overrideFontName = oneOverride["OverrideFontName"] as? String else { continue }
            ...
            if smallcaps {
                CoreTextFontDescriptor.setSmallCapsFontName(overrideFontName, forFontFamily: fontFamily, bold: bold, italic: italic)
            } else {
                CoreTextFontDescriptor.setOverrideFontName(overrideFontName, forFontFamily: fontFamily, bold: bold, italic: italic)
            }
        }
    }
}
...
open class CoreTextFontDescriptor: NSObject, NSCopying, NSCoding {
    ...
    @objc public class func fontDescriptor(withFontAttributes attributes: NSDictionary) -> CoreTextFontDescriptor {
    ...
    @objc public init(ctFontDescriptor: CTFontDescriptor) {
        ...
        let traitsDict = CTFontDescriptorCopyAttribute(ctFontDescriptor, kCTFontTraitsAttribute) as? NSDictionary
        ...
        if let familyName = CTFontDescriptorCopyAttribute(ctFontDescriptor, kCTFontFamilyNameAttribute) as? String {
    ...
    @objc public func fontAttributes() -> NSDictionary {
        ...
        let theSymbolicTraits = CTFontSymbolicTraits(rawValue: _stylisticTraits.rawValue | stylisticClass.rawValue)
        ...
        if !self.boldTrait && _needsChineseFontCascadeFix {
            let desc = CTFontDescriptorCreateWithNameAndSize("STHeitiSC-Light" as CFString, self.pointSize)
    ...
    @objc public func supportsNativeSmallCaps() -> Bool {
        if CoreTextFontDescriptor.smallCapsFontName(forFontFamily: fontFamily ?? "", bold: boldTrait, italic: italicTrait) != nil {
            return true
        }
        ...
        if let tmpFont = newMatchingFont() {
            if let fontFeatures = CTFontCopyFeatures(tmpFont) as? [[String: Any]] {
                for oneFeature in fontFeatures {
                    let featureTypeId = (oneFeature[kCTFontFeatureTypeIdentifierKey as String] as? NSNumber)?.intValue ?? 0

                    if featureTypeId == 3 { // Letter Case
                        if let featureSelectors = oneFeature[kCTFontFeatureTypeSelectorsKey as String] as? [[String: Any]] {
                            for oneSelector in featureSelectors {
                                let selectorId = (oneSelector[kCTFontFeatureSelectorIdentifierKey as String] as? NSNumber)?.intValue ?? 0
    ...
    @objc public var symbolicTraits: CTFontSymbolicTraits {
        ...
        set {
            _stylisticTraits = CTFontSymbolicTraits(rawValue: newValue.rawValue & ~CTFontSymbolicTraits.classMaskTrait.rawValue)
            stylisticClass = CTFontStylisticClass(rawValue: newValue.rawValue & CTFontSymbolicTraits.classMaskTrait.rawValue)
        }
    }
    ...
    private class func _createDictionaryOfAllAvailableFontOverrideNames(completion: @escaping @Sendable ([String: String]) -> Void) {
        DispatchQueue.global(qos: .background).async {
            ...
            for oneFontDescriptor in sortedFonts {
                let key = "\(oneFontDescriptor.fontFamily ?? "")-\(oneFontDescriptor.boldTrait ? 1 : 0)-\(oneFontDescriptor.italicTrait ? 1 : 0)-override"
    ...
    @objc public class func setFallbackFontFamily(_ fontFamily: String) {
        ...
        guard !fontFamily.isEmpty else {
            NSException(name: NSExceptionName(DTCoreTextFontDescriptorException as String), reason: "Fallback Font Family cannot be nil").raise()
            return
        }
        ...
        guard usedFontFamily == fontFamily else {
            NSException(name: NSExceptionName(DTCoreTextFontDescriptorException as String), reason: "Fallback Font Family '\(fontFamily)' not registered on the system").raise()
            return
        }
    ...
    @objc public class func setOverrideFontName(_ fontName: String, forFontFamily fontFamily: String, bold: Bool, italic: Bool) {
        _ensureInitialized()
        objc_sync_enter(_fontOverrides)
        defer { objc_sync_exit(_fontOverrides) }
    ...
    @objc public class func overrideFontName(forFontFamily fontFamily: String, bold: Bool, italic: Bool) -> String? {
        _ensureInitialized()
        objc_sync_enter(_fontOverrides)
        defer { objc_sync_exit(_fontOverrides) }
        let key = "\(fontFamily)-\(bold ? 1 : 0)-\(italic ? 1 : 0)-override"
    ...
    @objc public class func setSmallCapsFontName(_ fontName: String, forFontFamily fontFamily: String, bold: Bool, italic: Bool) {
        _ensureInitialized()
        objc_sync_enter(_fontOverrides)
        defer { objc_sync_exit(_fontOverrides) }
    ...
    @objc public class func smallCapsFontName(forFontFamily fontFamily: String, bold: Bool, italic: Bool) -> String? {
        _ensureInitialized()
        objc_sync_enter(_fontOverrides)
        defer { objc_sync_exit(_fontOverrides) }
        let key = "\(fontFamily)-\(bold ? 1 : 0)-\(italic ? 1 : 0)-smallcaps"
    ...
    private func _findOrMakeMatchingFont() -> CTFont? {
        ...
        if fontFamily != nil && fontName == nil {
            if smallCapsFeature {
                overrideName = CoreTextFontDescriptor.smallCapsFontName(forFontFamily: fontFamily ?? "", bold: boldTrait, italic: italicTrait)
            } else {
                overrideName = CoreTextFontDescriptor.overrideFontName(forFontFamily: fontFamily ?? "", bold: boldTrait, italic: italicTrait)
            }
        }
        ...
        if useFastFontCreation && ((fontName != nil && !(fontName?.hasPrefix(".") ?? false)) || overrideName != nil) {
            ...
            matchingFontDescriptors = CTFontDescriptorCreateMatchingFontDescriptors(searchingFontDescriptor!, mandatoryAttributes)

            if let descriptors = matchingFontDescriptors {
                let count = CFArrayGetCount(descriptors)
                if count == 1 {
                    matchingFontDescriptor = CTFontDescriptorCreateMatchingFontDescriptor(searchingFontDescriptor!, mandatoryAttributes)
                } else {
                    for i in 0..<count {
                        let currentFD = unsafeBitCast(CFArrayGetValueAtIndex(descriptors, i), to: CTFontDescriptor.self)
                        if let traits = CTFontDescriptorCopyAttribute(currentFD, kCTFontTraitsAttribute) as? [String: Any] {
            ...
            if matchingFontDescriptor == nil {
                ...
                matchingFontDescriptor = CTFontDescriptorCreateMatchingFontDescriptor(searchingFontDescriptor!, nil)
            }
            ...
            if matchingFontDescriptor == nil {
                ...
                matchingFontDescriptors = CTFontDescriptorCreateMatchingFontDescriptors(searchingFontDescriptor!, nil)
                if let descriptors = matchingFontDescriptors {
                    let count = CFArrayGetCount(descriptors)
                    if count == 1 {
                        matchingFontDescriptor = CTFontDescriptorCreateMatchingFontDescriptor(searchingFontDescriptor!, mandatoryAttributes)
