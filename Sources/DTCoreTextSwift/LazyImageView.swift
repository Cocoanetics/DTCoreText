...
//
  ...
  extension Notification.Name {
    ...
    public static let dtLazyImageViewWillStartDownload = Notification.Name(
      "DTLazyImageViewWillStartDownloadNotification")
    /// Posted when a lazy image view finishes downloading (check userInfo for errors).
    public static let dtLazyImageViewDidFinishDownload = Notification.Name(
      "DTLazyImageViewDidFinishDownloadNotification")
  }
  ...
  public class LazyImageView: UIImageView {
    ...
    private func performLoad(url: NSURL) async {
      ...
      if let existing = urlRequest {
        ...
        let mutable = NSMutableURLRequest(
          url: url as URL, cachePolicy: .returnCacheDataElseLoad, timeoutInterval: 10.0)
        urlRequest = mutable
    ...
    private func notifyDelegate() {
      delegate?.lazyImageView?(
        self, didChangeImageSize: CGSize(width: fullWidth, height: fullHeight))
    }
    ...
    private func handleDownloadFailure(error: Error) {
      ...
      NotificationCenter.default.post(
        name: .dtLazyImageViewDidFinishDownload, object: self, userInfo: userInfo)
    }
  }
