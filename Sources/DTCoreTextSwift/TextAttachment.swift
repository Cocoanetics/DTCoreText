...
open class TextAttachment: NSTextAttachment {
    ...
    @objc public class func textAttachment(with element: HTMLElement, options: NSDictionary?) -> TextAttachment? {
    ...
    public required init?(coder: NSCoder) {
        ...
        verticalAlignment = TextAttachmentVerticalAlignment(rawValue: UInt(coder.decodeInteger(forKey: "verticalAlignment"))) ?? .baseline
    }
    ...
    @objc public class func registerClass(_ theClass: AnyClass, forTagName tagName: String) {
        ...
        if let previousClass = registeredClass(forTagName: tagName) {
            NSLog("Replacing previously registered class '%@' for tag name '%@' with '%@'", NSStringFromClass(previousClass), tagName, NSStringFromClass(theClass))
        }
