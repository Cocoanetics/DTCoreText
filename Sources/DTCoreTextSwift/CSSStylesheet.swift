...
open class CSSStylesheet: NSObject, NSCopying {
    ...
    @objc public class func defaultStyleSheet() -> CSSStylesheet {
        ...
        if _defaultStylesheet == nil {
            ...
            if path == nil {
                let resourcesBundlePath = Bundle(for: self).path(forResource: "Resources", ofType: "bundle")
    ...
    func _uncompressShorthands(_ styles: NSMutableDictionary) {
        ...
        if let shortHand = styles["font"] as? String {
            ...
            let validFontWeights = ["bold", "bolder", "lighter", "100", "200", "300", "400", "500", "600", "700", "800", "900"]
            var fontWeightSet = false

            var fontSize = "normal"
            let validFontSizes = ["xx-small", "x-small", "small", "medium", "large", "x-large", "xx-large", "larger", "smaller"]
            var fontSizeSet = false

            let suffixesToIgnore = ["caption", "icon", "menu", "message-box", "small-caption", "status-bar", "inherit"]
        ...
        if let shortHand = styles["margin"] as? String {
            ...
            var top: String, right: String, bottom: String, left: String
            ...
            case 2:
                top = parts[0]; right = parts[1]; bottom = parts[0]; left = parts[1]
            default:
                top = parts[0]; right = parts[0]; bottom = parts[0]; left = parts[0]
            }
    ...
    private func _addStyleRule(_ rule: String, withSelector selectors: String) {
        ...
        for selector in split {
            ...
            if let colonRange = cleanSelector.range(of: ":") {
                let pseudoSelector = String(cleanSelector[cleanSelector.index(after: colonRange.lowerBound)...])
    ...
    @objc open func mergedStyleDictionary(for element: HTMLElement, matchedSelectors: AutoreleasingUnsafeMutablePointer<NSSet?>?, ignoreInlineStyle: Bool) -> NSDictionary? {
                ...
                if !selectorPart.isEmpty {
                    while let currentElement = nextElement {
                        ...
                        if selectorPart.hasPrefix("#") {
                            ...
                            let currentElementClassesString = (currentElement.attributes as? [String: Any])?["class"] as? String
                            let currentElementClasses = currentElementClassesString?.components(separatedBy: " ") ?? []
