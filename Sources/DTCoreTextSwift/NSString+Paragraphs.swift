...
public extension NSString {
    ...
    @objc func rangeOfParagraphsContaining(_ range: NSRange, parBegIndex: UnsafeMutablePointer<UInt>?, parEndIndex: UnsafeMutablePointer<UInt>?) -> NSRange {
        var beginIndex: CFIndex = 0
        var endIndex: CFIndex = 0

        CFStringGetParagraphBounds(self as CFString, CFRangeMake(range.location, range.length), &beginIndex, &endIndex, nil)
    ...
    @objc func indexIsAtBeginningOfParagraph(_ index: UInt) -> Bool {
    ...
    @objc func rangeOfParagraph(at index: UInt) -> NSRange {
        return rangeOfParagraphsContaining(NSRange(location: Int(index), length: 1), parBegIndex: nil, parEndIndex: nil)
    }

    /// Counts the number of paragraphs in the receiver.
    /// - Returns: The number of newline characters in the receiver
    @objc func numberOfParagraphs() -> UInt {
        var count: UInt = 0
