...
public class HTMLWriter: NSObject {
	...
	private func _tagRepresentation(forListStyle listStyle: CSSListStyle, closingTag: Bool, listPadding: CGFloat, inlineStyles: Bool) -> String {
		...
		if closingTag {
			...
			if listPadding > 0 {
				listStyleString += String(format: "-webkit-padding-start:%.0fpx;padding-left:%.0fpx;", listPadding, listPadding)
			}
	...
	private func _buildOutput(asHTMLFragment fragment: Bool) {
		...
		for i in 0..<paragraphs.count {
			...
			if paragraphRange.length > 0 {
				...
				paragraphFont = _attributedString.attribute(fontKey, at: paragraphRange.location, longestEffectiveRange: &fontEffectiveRange, in: paragraphRange) as! CTFont?
			} else {
				paragraphFont = nil
			}
			...
			let paraAttributes = _attributedString.attributes(at: paragraphRange.location, effectiveRange: nil)
			...
			_attributedString.enumerateAttributes(in: paragraphRange, options: []) { [self] (attributes, spanRange, stopEnumerateAttributes) in
				...
				if effectiveListStyle != nil && needsToRemovePrefix {
					...
					if prefixRange.location != NSNotFound {
						if NSMaxRange(prefixRange) < (plainSubString as NSString).length {
							plainSubString = (plainSubString as NSString).substring(from: NSMaxRange(prefixRange) - spanRange.location)
						} else {
				...
				let underlineKey = NSAttributedString.Key(rawValue: kCTUnderlineStyleAttributeName as String)
				...
				let localHTMLAttributes = attributesDict.object(forKey: DTCustomAttributesAttribute) as? NSDictionary
						...
						if let valueForParagraph = valueForParagraph {
							if fragment {
								...
								if let paragraphValue = valueForParagraph as? String {
									let paragraphClassComponents = paragraphValue.components(separatedBy: .whitespacesAndNewlines)
		...
		if !fragment {
			...
			output += "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01//EN\" \"http://www.w3.org/TR/html40/strict.dtd\">\n<html>\n<head>\n<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\" />\n<meta http-equiv=\"Content-Style-Type\" content=\"text/css\" />\n<meta name=\"Generator\" content=\"DTCoreText HTML Writer\" />\n<style type=\"text/css\">\n\(styleBlock)</style>\n</head>\n<body>\n"
		}

		if hasTab {
			if fragment {
				retString = retString.replacingOccurrences(of: "\t", with: "<span style=\"white-space:pre;\">\t</span>")
			} else {
				retString = retString.replacingOccurrences(of: "\t", with: "<span class=\"Apple-tab-span\">\t</span>")
			}
		}
