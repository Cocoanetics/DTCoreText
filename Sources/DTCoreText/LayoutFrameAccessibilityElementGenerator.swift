...
//
  ...
  public class LayoutFrameAccessibilityElementGenerator: NSObject {
    ...
    @objc
    public func accessibilityElements(
      for frame: CoreTextLayoutFrame, view: UIView,
      attachmentViewProvider block: @escaping AttachmentViewProvider
    ) -> [Any] {
      var elements = [Any]()
      ...
      for idx in 0..<paragraphRanges.count {
        let paragraphElements = accessibilityElements(
          inParagraphAt: idx, layoutFrame: frame, view: view, attachmentViewProvider: block)
    ...
    private func accessibilityElements(
      inParagraphAt index: Int, layoutFrame frame: CoreTextLayoutFrame, view: UIView,
      attachmentViewProvider block: @escaping AttachmentViewProvider
    ) -> [Any] {
      var elements = [Any]()

      guard let fragment = frame.attributedStringFragment() else { return elements }
      enumerateAccessibleGroups(in: frame, forParagraphAt: index) {
        attrs, substringRange, _, runs in
        if let element = self.accessibilityElement(
          for: fragment, at: substringRange, attributes: attrs, runs: runs, view: view,
          attachmentViewProvider: block)
        {
    ...
    private func enumerateAccessibleGroups(
      in frame: CoreTextLayoutFrame, forParagraphAt index: Int,
      using block: (NSDictionary, NSRange, UnsafeMutablePointer<ObjCBool>, [CoreTextGlyphRun]) ->
        Void
    ) {
      guard let paragraphRanges = frame.paragraphRanges, index < paragraphRanges.count else {
        return
      }
    ...
    private func accessibilityElement(
      for attributedString: NSAttributedString, at range: NSRange, attributes: NSDictionary,
      runs: [CoreTextGlyphRun], view: UIView, attachmentViewProvider block: AttachmentViewProvider
    ) -> Any? {
      if let attachment = attributes[NSAttributedString.Key.attachment] as? TextAttachment {
        return block(attachment)
      }
      return accessibilityElement(
        forText: attributedString, at: range, attributes: attributes, runs: runs, view: view)
    }

    private func accessibilityElement(
      forText attributedString: NSAttributedString, at range: NSRange, attributes: NSDictionary,
      runs: [CoreTextGlyphRun], view: UIView
