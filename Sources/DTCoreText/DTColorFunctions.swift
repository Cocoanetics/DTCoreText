...
public func DTColorCreateWithHexString(_ hexString: String) -> DTColor? {
  ...
  let redValue = _integerValueFromHexString(
    nsHex.substring(with: NSRange(location: 0, length: digits)))
  let greenValue = _integerValueFromHexString(
    nsHex.substring(with: NSRange(location: digits, length: digits)))
  let blueValue = _integerValueFromHexString(
    nsHex.substring(with: NSRange(location: 2 * digits, length: digits)))
...
public func DTColorCreateWithHTMLName(_ name: String) -> DTColor? {
  ...
  if name.hasPrefix("rgba") {
    ...
    let red =
      CGFloat((components[0].trimmingCharacters(in: .whitespaces) as NSString).floatValue) / 255.0
    let green =
      CGFloat((components[1].trimmingCharacters(in: .whitespaces) as NSString).floatValue) / 255.0
    let blue =
      CGFloat((components[2].trimmingCharacters(in: .whitespaces) as NSString).floatValue) / 255.0
