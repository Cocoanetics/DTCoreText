...
extension NSString {
  ...
  @objc public func isCSSLengthValue() -> Bool {
    ...
    guard let unit = scanner.scanCharacters(from: NSCharacterSet.dt_cssLengthUnitCharacterSet)
    else {
      return true
    }
  ...
  @objc public func pixelSizeOfCSSMeasure(
    relativeToCurrentTextSize textSize: CGFloat, textScale: CGFloat
  ) -> CGFloat {
  ...
  @objc public func dtEdgeInsets(relativeToCurrentTextSize textSize: CGFloat, textScale: CGFloat)
    -> DTEdgeInsets
  {
    ...
    if self.length > 0 {
      ...
      if parts.count == 4 {
        edgeInsets.top = (parts[0] as NSString).pixelSizeOfCSSMeasure(
          relativeToCurrentTextSize: textSize, textScale: textScale)
        edgeInsets.right = (parts[1] as NSString).pixelSizeOfCSSMeasure(
          relativeToCurrentTextSize: textSize, textScale: textScale)
        edgeInsets.bottom = (parts[2] as NSString).pixelSizeOfCSSMeasure(
          relativeToCurrentTextSize: textSize, textScale: textScale)
        edgeInsets.left = (parts[3] as NSString).pixelSizeOfCSSMeasure(
          relativeToCurrentTextSize: textSize, textScale: textScale)
        ...
        let paddingAmount = self.pixelSizeOfCSSMeasure(
          relativeToCurrentTextSize: textSize, textScale: textScale)
        edgeInsets = DTEdgeInsets(
          top: paddingAmount, left: paddingAmount, bottom: paddingAmount, right: paddingAmount)
      }
    }
  ...
  @objc public func arrayOfCSSShadows(
    withCurrentTextSize textSize: CGFloat, currentColor color: DTColor?
  ) -> [Any]? {
    ...
    while !scanner.isAtEnd {
      ...
      if let firstColor = scanner.scanHTMLColor() {
          ...
          let offsetYString = scanner.scanUpToCharacters(from: tokenEndSet)
        {
          ...
          let offsetX = (offsetXString as NSString).pixelSizeOfCSSMeasure(
            relativeToCurrentTextSize: textSize, textScale: 1.0)
          let offsetY = (offsetYString as NSString).pixelSizeOfCSSMeasure(
            relativeToCurrentTextSize: textSize, textScale: 1.0)
