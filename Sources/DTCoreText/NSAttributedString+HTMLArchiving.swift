...
extension NSAttributedString {
  ...
  private static func getArchivingDictionary(with attrs: [NSAttributedString.Key: Any])
    -> NSMutableDictionary
  {
  ...
  public func convertToData() -> Data? {
    let appendString = self.mutableCopy() as! NSMutableAttributedString
      ...
      if length > 0 {
        self.enumerateAttributes(in: NSRange(location: 0, length: length - 1), options: []) {
          attrs, range, _ in
          let typedAttrs = Dictionary(
            uniqueKeysWithValues: attrs.map {
              (NSAttributedString.Key(rawValue: $0.key.rawValue), $0.value)
            })
          ...
          if let attachment = attrs[.attachment] as? TextAttachment {
            ...
            if let imgPath {
              dict.setObject(
                imgPath, forKey: NSAttributedString.Key.attachment.rawValue as NSString)
              appendString.addAttribute(
                NSAttributedString.Key(rawValue: DTArchivingAttribute), value: dict, range: range)
            }
            appendString.removeAttribute(
              NSAttributedString.Key(rawValue: kCTRunDelegateAttributeName as String), range: range)
          }

          if let strokeColorRef = attrs[
            NSAttributedString.Key(rawValue: DTBackgroundStrokeColorAttribute)]
          {
            ...
            appendString.addAttribute(
              NSAttributedString.Key(rawValue: DTArchivingAttribute), value: dict, range: range)
            appendString.removeAttribute(
              NSAttributedString.Key(rawValue: DTBackgroundStrokeColorAttribute), range: range)
          }
        }
      }
    #endif
    ...
    do {
      return try NSKeyedArchiver.archivedData(
        withRootObject: appendString, requiringSecureCoding: false)
    } catch {
      return nil
    }
  }
  ...
  public static func attributedString(with data: Data) -> NSAttributedString? {
    ...
    appendString.enumerateAttributes(
      in: NSRange(location: 0, length: length - 1), options: .longestEffectiveRangeNotRequired
    ) { attrs, range, _ in
  ...
  public func htmlAttributes(at index: Int) -> [String: Any]? {
    return attribute(
      NSAttributedString.Key(rawValue: DTCustomAttributesAttribute), at: index, effectiveRange: nil)
      as? [String: Any]
  }
  ...
  public func rangeOfHTMLAttribute(_ name: String, at index: Int) -> NSRange {
    ...
    guard
      let attributes = attribute(
        NSAttributedString.Key(rawValue: DTCustomAttributesAttribute), at: index,
        effectiveRange: &rangeSoFar) as? [String: Any]
    else {
    ...
    while rangeSoFar.location > 0 {
      ...
      guard
        let extAttrs = attribute(
          NSAttributedString.Key(rawValue: DTCustomAttributesAttribute),
          at: rangeSoFar.location - 1, effectiveRange: &extendedRange) as? [String: Any],
        let extendedValue = extAttrs[name] as? NSObject,
        extendedValue.isEqual(value)
      else {
        break
      }
    ...
    while NSMaxRange(rangeSoFar) < length {
      ...
      guard
        let extAttrs = attribute(
          NSAttributedString.Key(rawValue: DTCustomAttributesAttribute), at: NSMaxRange(rangeSoFar),
          effectiveRange: &extendedRange) as? [String: Any],
        let extendedValue = extAttrs[name] as? NSObject,
        extendedValue.isEqual(value)
      else {
        break
